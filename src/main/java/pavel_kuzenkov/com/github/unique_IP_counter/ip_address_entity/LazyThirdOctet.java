...
class LazyThirdOctet implements Octet {
     ...
     * Сначала находим блок "ленивой" инициализации, куда нужно записать новое значение октета.
     * Делаем это путем деления нацело значения этого октета на 64 (количестко ячеек в каждом блоке),
     * Проверяем, не заполнен ли уже этот блок. Затем находим номер ячейки в этом блоке, куда необходимо записать
     * новое значение. Делаем это путём нахождения остатка от деления значения октета на 64. Если ячейка не занята -
     * создаём новый Octet и передаём ему массив с адресом. Если занята - проверяем не заполнен ли следующий октет
     * и передаем массив с адресом. После этого проверяем, не заполнился ли октет после добавления в него
     * новой записи. Если заполнился - увеличиваем счетчик соответствующего блока. Если счетчик равен 64(блок заполнился)
     * выставляем соотвествующий флаг и проверяем и удаляем (схлопываем) блок. Все следующие попытки записи в этот
     * блок будут пресекаться. Если заполнены все 4 блока - выставляем флаг заполненности у всего октета.
     *
    ...
    public boolean addNextOctet(int[] address) {
        ...
        Octet destinationOctet = destinationBlock[address[2] % 64]; //Т.к. 4 массива по 64 ячейки, чтобы узнать место в массиве назначения, нужно взять остаток от деления.
        ...
        if (!destinationOctet.isFull()) {
            ...
            if (destinationOctet.isFull()) { //Возможно после добавления, следующий октет заполнился.
                if ((fillCounters[address[2] / 64] = ++fillCounters[address[2] / 64]) >= 64) {//В принципе можно объеденить в один if.
